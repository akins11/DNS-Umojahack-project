...
from plotnine import theme_light, theme, element_blank, element_text
...
def add_pmCategory(df):
    """
    df [DataFrame] 
    """
    def PM_category(df, pm_col, aqi_col):
...
def create_ordered_text(data, cul, f_sort = True, rev = False, axis = "x", **kwargs):
    ...
    if cul not in data.columns.tolist():
        raise Exception(f"variable {count_col} is not in the data")
...
def rec_count(df, var, color = None, title = None, x_lab = None, rename_axis_text = None, typ = "plt"):
    ...
    f_plt = (
        ...
        labs(x = x_lab, y = "Count", 
             title = f"Counts By {var_l}" if title is None else f"Counts By {title}") +
        gg_comma_format("y") +
...
# Box Plot
def boxPlot(df, num_var, cat_var = None, zoom = None, color = None, title = None, rename_axis_text = None):
    """
     parameter
     ---------
     df        [DataFrame]
    ...
    if cat_var is None:
        color = "#056eee" if color is None else color
...
def facet_summary_plot(df, cat_var, num_var, fill, facet_by, 
    ...
    plt_size = (8, 5) if plt_size is None else plt_size
    xlab = vars_pltTitle[cat_var] if cat_var in vars_pltTitle.keys() else str.replace(cat_var, "_", " ").title()
    ylab = str.replace(num_var, "_", " ").title() if ylab is None else ylab
    
    f_plt = (ggplot(f_tbl, aes(f"reorder({cat_var}, {num_var})", num_var, fill = fill)) + 
             geom_col(show_legend = False) +
             geom_text(aes(label = num_var+"_prop"), position = position_dodge(width = 0.9), va = "bottom", size = 10, format_string = "{}%") +
             facet_wrap(facets = facet_by, ncol = n_col, scales = scales) +           
             gg_comma_format(axis = "y") + 
             labs(x = xlab, y = ylab, title = title) +
             theme_light() +
             theme(figure_size = plt_size, axis_text_x = element_text(angle = angle))
            )
    if colors is not None:
        f_plt = f_plt + scale_fill_manual(values = colors) 
